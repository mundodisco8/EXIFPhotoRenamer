...
from ui.renameWarningDialog import Ui_renameWarningDialog
...
class Window(QWidget, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.toFixDatesTableView.selectionModel().selectionChanged.connect(self.updateListOfDates)
        # I want to select a date for a dateless item by clicking the row number on the available dates list. To do this
        # I have to override the key press event of the widget with my own keyPressEvent method
        self.toFixDatesTableView.keyPressEvent = self.selectDateWithHotkey
        ...
        # There's no signal in a QTableViwe for a change in selected cell, but there is one in the selection model
        # associated to it. Keep in mind that the selection happens in datesTableView, not toFixDatesTableView!
        self.datesTableView.selectionModel().selectionChanged.connect(self.dateSelected)
    ...
    def renameFiles(self) -> None:
        ...
        """
        # First Check that none of the proposed filenames would overwrite a previously existing file (on Windows,
        # Path().rename raises an exception before overwriting, but not on Linux)
        filesToRename: list[str] = self.toRenameModel.returnColumn(0)
        mediaFilesToRename: list[MediaFile] = [
            mediaFile for mediaFile in self.mediaFileList if str(mediaFile.fileName) in filesToRename
        ]
        clashList = [x.newName for x in mediaFilesToRename if x.newName and x.newName.is_file()]
        if clashList:
            # Show a Warning dialog
            dlg = renameFilesWarningDialog(self)
            numberClashes = len(clashList)
            if numberClashes == 1:
                dlg.messageTxt.setText("The proposed new names clash with one existing file")
                dlg.fileListTxt.setText("Here's the file that clashes")
                dlg.fileListTextEdit.setPlainText(str(clashList[0]))
            else:
                dlg.messageTxt.setText(f"The proposed new names clash with {numberClashes} existing files")
                dlg.fileListTxt.setText("Here are the files that clash")
                dlg.fileListTextEdit.setPlainText("\n".join(str(x) for x in clashList))
            dlg.exec()
        else:
            # Only rename of no files clash!
            for mediaFile in mediaFilesToRename:
                # It should not be possible to have a clash, but having that extra check at the end doesn't hurt
                if mediaFile.newName and mediaFile.fileName != mediaFile.newName and not mediaFile.newName.is_file():
    ...
    def selectDateWithHotkey(self, event: QKeyEvent) -> None:
        """Selects a date from the list of dates available when pressing a number key, in the Fix Date tab

        Replaces the keyPressEvent of the toFixDatesTableView QTableWidget with a custom one that will select one of the
        items on datesTableView by row order when pressing a number key

        Args:
            event (): the key press event that triggers this event

        Returns:
            bool: _description_
        """
        text: str = event.text()
        if len(text) == 1 and text.isdigit() and int(text) <= self.datesTableView.model().rowCount():
            # If key was a number from 0 to 9, and the number was equal or lesser than the number of rows available in
            # the datesTableView
            self.progressBarTxt.setText(text)
            self.datesTableView.selectRow(int(text) - 1)  # rows are 0-indexed
    ...
    def dateSelected(self) -> None:
        ...
        It also runs one one of the columns of the table is selected via hotkey
        """
        index = self.datesTableView.selectionModel().currentIndex()
        if not index.isValid():
            # If the index is out of range (e.g. like when we try to clear the selection), just return
            return
        ...
        # Finally, clear the selection or the next file we click on might take the date on this row. We do this by
        # setting the current index to an empty item?
        # NOTE: this will trigger an update too, that we have to block
        self.datesTableView.setCurrentIndex(QModelIndex())
        # And if we advance to the next item in the toFixDates table, then we can get new dates and the whole process
        # will be snappier, wheter it's through clicking or hot-key selection
        self.toFixDatesTableView.selectRow(self.toFixDatesTableView.currentIndex().row() + 1)
        # This will trigger a selectionChanged in the selectionModel of the View
...
class renameFilesWarningDialog(QDialog, Ui_renameWarningDialog):
    """A custom warning dialog shown when there's a risk of file overwrites during the rename process

    It displays a list of the files that are clashing
    """
